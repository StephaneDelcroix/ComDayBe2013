...
//		}
        ...
        public static Session GetSession (int id)
        {
            ...
            lock (locker) {
				...
				var speakerKeys = new List<string> ();
				foreach (var ss in me.Table<SessionSpeaker> ())
					if (ss.SessionKey == session.Key)
						speakerKeys.Add (ss.SpeakerKey);
				session.SpeakerKeys = speakerKeys;
		...
		{
			lock (locker) {
				return (from s in me.Table<UserGroup> ()
				        where s.ID == id
				        select s).FirstOrDefault ();
			}
		}
